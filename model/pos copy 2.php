...
public function saveLoc($loc_name,$cood,$postal,$locality,$state,$country){
  ...
  try
  {
   $stmt = $db->prepare("call saveLoc2(:loc_name,:cood,:postal,:locality,:state,:country)");
   $stmt->bindParam("loc_name",$loc_name);
   $stmt->bindParam("cood",$cood);
   $stmt->bindParam("postal",$postal);
   $stmt->bindParam("locality",$locality);
...
public function saveCur($cur_name,$currency_type,$amount,$addresscur,$msisdncur,$response){
  $db = $this->dblocal;
  try
  {
   $stmt = $db->prepare("call saveCur(:cur_name,:currency_type,:amount,:addresscur,:msisdncur,:response)");
