...
if ($err) {
  ...
  //echo $response;
  echo "Balance :".$bal = str_replace('"'," ",$responsebal);
...
echo "Coupon Price outside pass:".$price = $_POST['price']*1000000000000000000;
	if (($u_pass1 == $passreq)&&($bal > $cover_value || $bal == $cover_value)){
		//	$response = null;
	//system("ping -c 1 google.com", $response);
	//if($response == 0)
	//{
		echo "Coupon Price-inside pass :".$price = $_POST['price']*1000000000000000000;
			...
			curl_setopt_array($curl, array(
			  ...
			  if ($err) {
				...
				// -----------Publickey,Privatekey,Address----------------
				echo "order address: :".$address2 = ($result3['address']);
    ...
    echo "addgorder rpt :". $receipt2 = str_replace('"',"",$response2);
		 ...
		 //echo "<br> Reciept query :".$response3."<br>";
			...
			$json_o = json_decode($response3,true);
					...
					if ($a != 100){
                        ...
                        echo "addgorder status :". $out .=$c."-".$a."-Success,"; 
					}
					//echo $d."-d-Error,"; 
					}
